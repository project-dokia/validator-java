...
/* harmony import */ var _service_sync_service__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../service/sync.service */ "./src/service/sync.service.ts");
var __decorate = (undefined && undefined.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
var AppComponent = /** @class */ (function () {
    function AppComponent(syncService, httpStatus, _router) {
        var _this = this;
    ...
    AppComponent.prototype.sync = function () {
        ...
        this.syncService.syncObservable()
            ...
            alert("Syncronized!");
            _this.getLastSyncObservable();
        }, function (err) {
            console.log(err);
        });
    };
    AppComponent.prototype.getLastSyncObservable = function () {
        var _this = this;
        this.syncService.getLastSyncObservable()
            ...
            if (response.lastSync != undefined && response.lastSync != null && response.lastSync != "null") {
                _this.lastSync = response.lastSync;
            }
            else {
                _this.lastSync = "SINCRONIZAR - URGENTE";
            }
        }, function (err) {
            console.log(err);
        });
    ...
    AppComponent = __decorate([
        Object(_angular_core__WEBPACK_IMPORTED_MODULE_0__["Component"])({
            ...
            providers: [_service_sync_service__WEBPACK_IMPORTED_MODULE_3__["SyncService"]]
        }),
        __metadata("design:paramtypes", [_service_sync_service__WEBPACK_IMPORTED_MODULE_3__["SyncService"],
            _service_interceptor_module__WEBPACK_IMPORTED_MODULE_2__["HTTPStatus"],
...
var RuleComponent = /** @class */ (function () {
    ...
    RuleComponent.prototype.saveUpdate = function () {
    ...
    RuleComponent.prototype.view = function (_id, rule) {
        ...
        if (_id != undefined) {
            this.ruleService.getRuleByIdObservable(_id)
                ...
                _this.importantAcceptancePercentage = res.importantAcceptancePercentage;
                _this.acceptancePercentage = res.acceptancePercentage;
                _this.idRuleUpdate = res._id;
                    ...
                    for (var _a = 0, _b = _this.fieldsSelected; _a < _b.length; _a++) {
                        ...
                        if (field._id == fieldSelected._id) {
                            if (field.idType == "fecde76de43641609a7da3a6a2014642") {
                                var contains = false;
                                for (var _c = 0, _d = _this.fieldsRuleViewNenhum; _c < _d.length; _c++) {
                                    var field_1 = _d[_c];
                                    if (field_1._id == fieldSelected._id) {
                                        contains = true;
                                    }
                                }
                                if (contains == false) {
                                ...
                                var contains = false;
                                for (var _e = 0, _f = _this.fieldsRuleView; _e < _f.length; _e++) {
                                    var field_2 = _f[_e];
                                    if (field_2._id == fieldSelected._id) {
                                        contains = true;
                                    }
                                }
                                if (contains == false) {
                ...
                for (var _g = 0, fields_2 = fields; _g < fields_2.length; _g++) {
                    var field = fields_2[_g];
                    for (var _h = 0, _j = _this.fieldsNenhum; _h < _j.length; _h++) {
                        var fieldSelected = _j[_h];
                        if (field._id == fieldSelected._id) {
                            var contains = false;
                            for (var _k = 0, _l = _this.fieldsRuleViewNenhum; _k < _l.length; _k++) {
                                var field_3 = _l[_k];
                                if (field_3._id == fieldSelected._id) {
                                    contains = true;
                                }
                            }
                            if (contains == false) {
...
var ValidatorComponent = /** @class */ (function () {
    ...
    ValidatorComponent.prototype.addValidator = function (index) {
        ...
        for (var _i = 0, _a = this.rules; _i < _a.length; _i++) {
            ...
            if (count == index) {
                this.rule = rule;
                var fields = this.rule.fields;
                this.rule.fields = Array();
                for (var _b = 0, fields_1 = fields; _b < fields_1.length; _b++) {
                    var field = fields_1[_b];
                    var contains = false;
                    for (var _c = 0, _d = this.rule.fields; _c < _d.length; _c++) {
                        var fieldRule = _d[_c];
    ...
    ValidatorComponent.prototype.validator = function () {
        ...
        this.validatorService.insertValidatorObservable(document).subscribe(function (res) {
            var result = Array();
            result = res.result;
            _this.resultDokia = String(res.resultDokia);
            _this.resultPercentageImportant = String(res.resultPercentageImportant);
            _this.importantAcceptancePercentage = String(res.importantAcceptancePercentage);
            _this.resultPercentage = String(res.resultPercentage);
            _this.acceptancePercentage = String(res.acceptancePercentage);
            var count = 0;
...
var RuleService = /** @class */ (function () {
    ...
    RuleService.prototype.updatePercentageObservable = function (idRule, importantAcceptancePercentage, acceptancePercentage) {
        return this.http.post(this.ruleURL + "/update-percentage/" + idRule + "/" + importantAcceptancePercentage + "/" + acceptancePercentage, {});
    };
...
var SyncService = /** @class */ (function () {
    function SyncService(http) {
        this.http = http;
        this.headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpHeaders"]().set('Content-Type', 'application/json');
        this.syncURL = _environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].apiUrl + '/sync';
    }
    SyncService.prototype.syncObservable = function () {
        return this.http.get(this.syncURL);
    };
    SyncService.prototype.getLastSyncObservable = function () {
        return this.http.get(this.syncURL + "/lastSync");
    };
